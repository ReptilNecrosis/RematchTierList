    ...
    <>
      ...
      <div className="record-list">
        {rows.map((row) => {
          ...
          return (
            <div key={row.tierId} className="record-row team-profile-record-row">
...
export function TeamProfileScreen({
  ...
  const selectedSeasonRecord = seasonRecords.find((record) => record.seasonKey === selectedSeasonKey) ?? null;
  const crossTierWins = (teamStats?.oneTierUpWins ?? 0) + (teamStats?.twoTierUpWins ?? 0);
  ...
  return (
    <div className="page team-profile-page">
      ...
      <div className="profile-top team-profile-premium" style={profileAccentStyle}>
        <section className="profile-header team-profile-hero">
          <div className="team-profile-hero-main">
            ...
            <div className="team-profile-title-row">
              <div className="profile-avatar team-profile-avatar" aria-hidden={!team.logoUrl || undefined}>
                {team.logoUrl ? (
                  <Image
                    src={team.logoUrl}
                    alt={team.name}
                  ...
                  <span className="team-profile-avatar-fallback">{getTeamInitials(team.name)}</span>
                )}
              </div>
            ...
            <div className="team-profile-meta-row">
              ...
              <span className="team-profile-chip">{team.verified ? "Roster verified" : "Needs verification"}</span>
              <span className="team-profile-chip">Added by {team.addedBy}</span>
              <span className="team-profile-chip">{new Date(team.createdAt).toDateString()}</span>
            </div>

            <div className="profile-sub team-profile-sub">
              Performance snapshot for {team.name} across the live ladder, confirmed match history, and seasonal form.
            </div>

            {teamPendingFlag ? (
              <div className="team-profile-highlight">
                Pending rules signal: {teamPendingFlag.movementType} because {teamPendingFlag.reason.replaceAll("_", " ")}.
              </div>
            ) : null}

            <div className="profile-stats team-profile-stats-grid">
              <div className="ps team-profile-stat-card">
                <div className="ps-val accent-green">{Math.round((selectedSeasonRecord?.overallWinRate ?? 0) * 100)}%</div>
          ...
          <aside className="profile-season-sidebar team-profile-season-panel">
            <div className="team-profile-panel-head">
              <div>
                <div className="team-profile-panel-kicker">Season Intelligence</div>
              ...
              {seasonRecordView === "month" ? (
                seasonRecords.map((record) => (
                  <Link
                    ...
                    className={`season-card season-card-link team-profile-season-card ${
                      record.seasonKey === selectedSeasonKey ? "active" : ""
                    }`}
                  >
                    <div className="team-profile-season-card-topline">
                      <div className="season-card-title">{record.seasonLabel}</div>
                      <div className="team-profile-season-rate">{Math.round(record.overallWinRate * 100)}%</div>
                    </div>
                    <div className="season-card-meta">
                      {record.wins}-{record.losses} · {record.seriesPlayed} series
                    </div>
      ...
      <div className="profile-grid team-profile-data-grid">
            ...
            selectedSeasonSeries.length === 0 ? (
              ...
              <div className="season-history-list">
                {selectedSeasonSeries.map((entry) => (
                  <div key={entry.id} className="history-item team-profile-history-item">
        ...
        <LinkedAccordionPair
          ...
          leftChildren={history.map((entry) => (
            <div key={entry.id} className="history-item team-profile-history-item">
              ...
              <div className="h-info">
                <div className="p-name">
                  {entry.movementType === "placement" ? "Placed" : entry.movementType === "promotion" ? "Promoted" : "Moved"}{" "}
        ...
        <AccordionCard title="Head to Head" icon="H2H" className="full-span h2h-dropdown-card">
          <HeadToHeadSearch
            teams={allTeams}
            allSeries={allSeries}
