...
async function persistConfirmedImport(args: {
  ...
  if (args.createdUnverifiedNames.length > 0) {
    ...
    // Auto-place teams that now have 3+ distinct tournament appearances into Tier 7 (verified = false)
    const uniqueNormalizedNames = [...new Set(args.createdUnverifiedNames.map(normalizeName))];
    const { data: allAppearances } = await client
      .from("unverified_appearances")
      .select("team_name, normalized_name, tournament_id")
      .in("normalized_name", uniqueNormalizedNames);

    const distinctTournamentsByName = new Map<string, { name: string; tournaments: Set<string> }>();
    for (const row of (allAppearances ?? []) as Array<Record<string, string>>) {
      const key = row.normalized_name;
      const existing = distinctTournamentsByName.get(key) ?? { name: row.team_name, tournaments: new Set() };
      existing.tournaments.add(row.tournament_id);
      distinctTournamentsByName.set(key, existing);
    }

    const eligibleForAutoPlacement = [...distinctTournamentsByName.entries()]
      .filter(([, entry]) => entry.tournaments.size >= 3)
      .map(([normalizedName, entry]) => ({ normalizedName, name: entry.name }));

    if (eligibleForAutoPlacement.length > 0) {
      const { data: existingTeams } = await client
        .from("teams")
        .select("id, name, verified")
        .in("slug", eligibleForAutoPlacement.map(({ normalizedName }) => normalizedName.replace(/\s+/g, "-")));

      const existingNames = new Set(
        ((existingTeams ?? []) as Array<Record<string, string>>).map((t) => normalizeName(t.name))
      );

      const toInsert = eligibleForAutoPlacement.filter(({ normalizedName }) => !existingNames.has(normalizedName));

      if (toInsert.length > 0) {
        const teamRows = toInsert.map(({ name }) => ({
          slug: normalizeName(name).replace(/\s+/g, "-"),
          name,
          short_code: name.slice(0, 2).toUpperCase(),
          current_tier_id: "tier7",
          verified: false,
          created_by: args.actorAdminId ?? null
        }));

        const { data: insertedTeams, error: teamInsertError } = await client
          .from("teams")
          .insert(teamRows as never)
          .select("id, name");

        if (teamInsertError) {
          throw new Error(`Could not auto-place unverified teams: ${teamInsertError.message}`);
        }

        if (insertedTeams && (insertedTeams as Array<Record<string, string>>).length > 0) {
          const historyRows = (insertedTeams as Array<Record<string, string>>).map((team) => ({
            team_id: team.id,
            from_tier_id: null,
            to_tier_id: "tier7",
            movement_type: "placement",
            reason: "Auto-placed after 3 tournament appearances",
            created_by: args.actorAdminId ?? null
          }));

          const { error: historyError } = await client.from("team_tier_history").insert(historyRows as never);
          if (historyError) {
            throw new Error(`Could not save tier history for auto-placed teams: ${historyError.message}`);
          }
        }
      }
    }
